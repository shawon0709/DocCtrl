...
import { RowData } from "@/components/utils/FromEmployerGoogleSheet";
import { FromEmployerUseData } from "@/components/classes/FromEmployerDataProvider";
import { IoMdClose } from "react-icons/io";

// Define props interface
interface FromEmployerProps { }

const FromEmployer: React.FC<FromEmployerProps> = () => {
    const { fromEmployerLoading, fromEmployerData, fromEmployerError, fetchFromEmployerData } = FromEmployerUseData();
    ...
    const columnDefs: (ColDef | ColGroupDef)[] = [
        ...
        { headerName: 'Reference No.', field: 'refNo', filter: 'agTextColumnFilter', width: 250, wrapText: true, pinned: 'left', floatingFilter: true, resizable: true, sortable: true, autoHeight: true, cellStyle: { display: 'flex', alignItems: 'center' } },
        { headerName: 'Title', field: 'title', filter: 'agTextColumnFilter', width: 300, wrapText: true, floatingFilter: true, resizable: true, sortable: true, autoHeight: true },
    ...
    useEffect(() => {
        if (!fromEmployerLoading && fromEmployerData) {
            setRowData(fromEmployerData);
            setLoading(false);
        } else if (fromEmployerError) {
            setErrors(fromEmployerError);
            setLoading(false);
        }
    }, [fromEmployerLoading, fromEmployerData, fromEmployerError]);
        ...
        <div className={`transition-opacity duration-1000 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
            <div className="flex flex-wrap">
                <div className="w-full md:w-2/3">
                    ...
                    <Spin spinning={fromEmployerLoading} size="large">
